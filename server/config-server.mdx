...
<ParamField path="transport" type="string" default='"websocket"'>
  ...
  | `"lengthdelimitedl"` | Little-endian `u32` length-delimited codec (see [tokio-util](https://docs.rs/tokio-util/latest/tokio_util/codec/length_delimited/index.html)) — lower overhead, not WebSocket |
...
<ParamField path="runtime" type="string" default='"multithread"'>
  ...
  `"multithread"` is the right choice for the vast majority of deployments. Switch to `"threadpercore"` only after profiling shows CPU saturation with `"multithread"`.
</ParamField>
